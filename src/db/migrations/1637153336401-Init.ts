...
export class Init1637153336401 implements MigrationInterface {
    name = 'Init1637153336401'
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE TABLE "purchases" ("id" SERIAL NOT NULL, "createDate" TIMESTAMP NOT NULL DEFAULT now(), "updateDate" TIMESTAMP NOT NULL DEFAULT now(), "deleteDate" TIMESTAMP, "status" "public"."purchases_status_enum" NOT NULL DEFAULT 'PENDING', "quantity" integer NOT NULL DEFAULT '1', "customerId" integer NOT NULL, "itemId" integer NOT NULL, CONSTRAINT "PK_1d55032f37a34c6eceacbbca6b8" PRIMARY KEY ("id"))`);
        await queryRunner.query(`CREATE TABLE "users" ("id" SERIAL NOT NULL, "createDate" TIMESTAMP NOT NULL DEFAULT now(), "updateDate" TIMESTAMP NOT NULL DEFAULT now(), "deleteDate" TIMESTAMP, "role" "public"."users_role_enum" NOT NULL DEFAULT 'CUSTOMER', "login" text NOT NULL, "password" character varying NOT NULL, "balance" numeric NOT NULL DEFAULT '300', CONSTRAINT "UQ_2d443082eccd5198f95f2a36e2c" UNIQUE ("login"), CONSTRAINT "PK_a3ffb1c0c8416b9fc6f907b7433" PRIMARY KEY ("id"))`);
        await queryRunner.query(`CREATE TYPE "public"."cards_type_enum" AS ENUM('MASTERCARD', 'VISA')`);
        await queryRunner.query(`CREATE TABLE "cards" ("id" SERIAL NOT NULL, "createDate" TIMESTAMP NOT NULL DEFAULT now(), "updateDate" TIMESTAMP NOT NULL DEFAULT now(), "deleteDate" TIMESTAMP, "type" "public"."cards_type_enum" NOT NULL, "number" character varying NOT NULL, "expired" character varying NOT NULL, "cvv" integer NOT NULL, "userId" integer NOT NULL, CONSTRAINT "PK_5f3269634705fdff4a9935860fc" PRIMARY KEY ("id"))`);
